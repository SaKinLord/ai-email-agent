...
def load_config(filepath="config.json"):
    """Loads configuration from a JSON file and validates required keys."""
    # Keep this function here as it might be useful for standalone testing
    # or if config needs reloading within specific logic later.
    try:
        with open(filepath, 'r') as f:
            config_data = json.load(f)
        logging.info(f"Configuration loaded successfully from {filepath}")
        required_keys = ['gmail', 'llm', 'classification', 'database', 'ml', 'retraining']
        missing_keys = [key for key in required_keys if key not in config_data]
        if missing_keys:
            logging.error(f"Configuration file '{filepath}' is missing required top-level keys: {missing_keys}")
...
# --- MODIFIED authenticate_gmail ---
def authenticate_gmail(token_gcs_bucket, token_gcs_path, credentials_path, scopes, storage_client_instance=None):
        ...
        if not creds or not creds.valid:
            if creds and creds.expired and creds.refresh_token:
                ...
                try:
                    ...
                    logging.error(f"Token refresh failed: {e}. Cannot proceed without interactive flow.")
...
2. "purpose": A string matching one of these exact categories: "Action Request", "Information", "Question", "Meeting Invite", "Promotion", "Social", "Notification", "Spam", "Other".
...
3. "reasoning": A short explanation why this action is appropriate based on the email content/analysis.
    ...
    try:
        ...
        if message and message.content and len(message.content) > 0:
            ...
            try:
                ...
                logging.warning(f"Could not parse action suggestions JSON: {cleaned_json_text}")
                return []
        else:
            logging.warning("Empty or missing response from Anthropic for action suggestions")
            return []
    except anthropic.APIConnectionError as e:
         logging.error(f"Anthropic API Connection Error during action suggestion: {e}")
         # No retry here for simplicity, could add if needed
    except anthropic.RateLimitError as e:
         logging.error(f"Anthropic Rate Limit Error during action suggestion: {e}")
    except anthropic.APIStatusError as e:
         logging.error(f"Anthropic API Status Error during action suggestion: status_code={e.status_code}, response={e.response}")
    except Exception as e:
        logging.error(f"Error generating action suggestions: {type(e).__name__}: {e}", exc_info=True)
...
def process_email_with_memory(email_data, llm_client, config, memory=None, feedback_history=None, ml_pipeline=None, ml_label_encoder=None):
    ...
    # 1. Classify and get enhanced analysis
    ...
    if analysis_result and isinstance(analysis_result, dict):
        ...
        logging.debug(f"Email {processed_data.get('id')} analysis failed or missing, defaults set.")
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            try:
                ...
                logging.error(f"Failed to refresh calendar token: {e}. Need re-authentication.", exc_info=True)
...
def get_calendar_service(storage_client_instance, token_gcs_bucket, credentials_path):
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            try:
                ...
                logging.error(f"Failed to refresh calendar token: {e}", exc_info=True)
